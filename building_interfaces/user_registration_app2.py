...
def loadUserInfo():
    ...
    if user.retrieve(pword):
        messageTitle = f"{user.firstName} {user.lastName} Account"
        messageContent = (
            f"Account found for {user.firstName} {user.lastName}: {user.dob}"
        )
...
instructions = Label(
    mainWindow, text="Complete the form and press submit to register a new user."
)
